...
This repository hosts a set of questions designed to compare medical VQA models. 

The benchmarrk is Inspired by radiologists' workflow who inspect 3D CT-scans by sliding through their axial slices. We built a set
of questions each bound to a specific slice within a given CT-scan. We have also built multi-scan questions that
require to compare a pair of slices from two different volumes. The questions in this dataset focus
on task requiring fine-grained spatial analysis of the images (e.g. measurements).
While each question focuses on a specific axial slice containing the necessary information to answer it, the full CT scan remains available for additional context.
...
* `is_larger` e.g., "Is gall bladder larger than postcava?"
